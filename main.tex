...
% \author{\IEEEauthorblockN{Mufan Xiang}
% \IEEEauthorblockA{
...
\author{
\IEEEauthorblockN{Mufan Xiang\IEEEauthorrefmark{2}, Yongjian Li\IEEEauthorrefmark{3}$^*$ \thanks{ $^*$Corresponding authors}, Yongxin Zhao\IEEEauthorrefmark{2}\IEEEauthorrefmark{3}$^*$ }
...
\IEEEauthorblockA{\IEEEauthorrefmark{3} State Key Laboratory of Computer Science, Institute of Software, Chinese Academy of Sciences, Beijing, China}
}
...
% 然而，由于在第一节中提到的原因，即 Chisel 生成的 SystemVerilog 代码可读性差，因此无法直接利用 RISC-V Formal 框架来验证 Chisel 芯片。
...
% 除了 Chisel，还有一些其他用于生成 HDL 的语言。SpinalHDL 与 Chisel 类似，在 Scala 层次上构建的 HDL 生成工具，同时有对基本形式化性质描述支持和工具调用。Amaranth HDL 则选择使用 Python 来构建硬件。
...
% 同时，我们将 ChiselFV 应用到更多的开源 Chisel 模块中进行形式化验证，以不断提高 ChiselFV 的实用性。
